...
namespace DATN_Infrastructure.Repository
{
    public class AccountReponsitory : GenericeReponsitory<Account>, IAccountReponsitory
    {
        ...
        public async Task<ReturnAccountDTO> GetAllAsync(BrandParams brandParams)
        {
            var result = new ReturnAccountDTO();
            var query = await _context.Accounts!.Where(p => p.Role == 1 ||
            p.Role == 2).AsNoTracking().ToListAsync();
        ...
        public async Task<ReturnAccountDTO> GetAllUsers(string trangthai, BrandParams brandParams)
        {
            var result = new ReturnAccountDTO();
            var accountList =  _context.Accounts!.Where(p => p.Role == 0).AsNoTracking();
            if (trangthai == "Hoạt động")
            {
                accountList = accountList.Where(p => p.Login.Any(x => x.Action == "Hoạt động"));
            }
            else if (trangthai == "Chờ xác nhận")
            {
                accountList = accountList.Where(p => p.Login.Any(x => x.Action == "Chờ xác nhận"));
            }
            else if (trangthai == "Đã bị khóa")
            {
                accountList = accountList.Where(p => p.Login.Any(x => x.Action == "Đã bị khóa"));
            }
            var query = await accountList.ToListAsync();
            if (!string.IsNullOrEmpty(brandParams.Search))
            {
                query = query.Where(p => p.FullName.ToString()
                .ToLower().Contains(brandParams.Search)).ToList();
        ...
        public async Task<int> LockAccount(int idaccount, string lydo)
        {
            var tk = await _context.Logins!.FirstOrDefaultAsync(a => a.AccountId == idaccount);
            if (tk == null)
            {
        ...
        public async Task<int> OpenAccount(int idaccount)
        {
            var tk = await _context.Logins!.FirstOrDefaultAsync(a => a.AccountId == idaccount);
            if (tk == null)
            {
            ...
            tk.Action = "Hoạt động";
            tk.TimeStamp = DateTime.Now;
            tk.Description = "không có ghi chú";
            _context.Logins.Update(tk);
            await _context.SaveChangesAsync();
            return tk.Id;
        }
